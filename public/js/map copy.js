...
function downloadAllVisibleGeoJSON() {
  ...
  [pointLayer, lineLayer, polygonLayer, multiPolygonLayer].forEach(group => {
    group.eachLayer(layer => {
      if (map.hasLayer(layer)) {
        ...
        // Ajouter tous les attributs du lieu correspondant
        for (const lieu of lieux) {
          if (markers[lieu.id] === layer) {
            geoJSON.properties = { ...lieu }; // ✅ tous les attributs
            break;
          }
        }

        // Sécurité : s'il n'a pas été trouvé
        if (!geoJSON.properties) {
          geoJSON.properties = { nom: "Inconnu" };
        }
  ...
  downloadBlob(JSON.stringify(geojson, null, 2), 'all_visible.geojson', 'application/json');
}


function downloadAllVisibleExcel() {
  const geojson = getAllVisibleGeoJSON();

  // Convertir chaque feature en une ligne Excel avec tous les attributs
  const rows = geojson.features.map(f => {
    const row = { ...f.properties }; // tous les attributs
  ...
  XLSX.writeFile(workbook, "all_visible.xlsx");
}

function downloadAllVisibleKML() {
  const geojson = getAllVisibleGeoJSON();

  // La lib tokml accepte properties automatiquement
  const kml = tokml(geojson, {
    name: 'nom', // facultatif : quel champ afficher comme nom dans Google Earth
    description: (props) => {
      // Créer une description à partir de tous les attributs
  ...
  downloadBlob(kml, 'all_visible.kml', 'application/vnd.google-earth.kml+xml');
}
function downloadAllVisibleShapefile() {
  const geojson = getAllVisibleGeoJSON();

  // shpwrite supporte automatiquement les properties
...
function updateResultTable(features) {
  ...
  const downloadBox = document.getElementById('download-buttons');
